...
void loop() {
  ...
  // ===== Sensor Readings =====
  int soundRaw = analogRead(soundSensorPin);
  int soundDB = map(soundRaw, 0, 4095, 0, 130);  // Corrected for ESP32 ADC

  // ===== Print and Send All Sensor Data =====
  String data = "";
  data += "Sound Level (dB): " + String(soundDB);
...
float readVoltage() {
  ...
  float voltage = (float)rawValue / 4095.0 * 3.3; // Use 3.3V for ESP32 ADC
